...
public class XMLSchemaLoader implements SchemaLoader {
	...
	public XMLSchemaLoader(String schemaFile, String ruleFile) {
		...
		//释放ruleLoader
		ruleLoader = null;
		this.dataHosts = RedisShchemaLoader.load("dataHost:*", DataHostConfig.class);
		this.dataNodes = RedisShchemaLoader.load("dataNode:*", DataNodeConfig.class);
		this.schemas = RedisShchemaLoader.load("schema:*", SchemaConfig.class);
		//读取加载schema配置

		if(dataHosts.isEmpty() || dataNodes.isEmpty() || schemas.isEmpty()) {
	...
	private void loadSchemas(Element root) {
		...
		for (int i = 0, n = list.getLength(); i < n; i++) {
			...
			RedisUtils.set("schema:" + name, schemaConfig);
		}
	}
	...
	private void createDataNode(String dnName, String database, String host) {
		...
		RedisUtils.set("dataNode:" + conf.getName(), conf);
	}
